...
def get_topk_accuracy(y_true: np.ndarray, y_pred: np.ndarray, k: int = 3, include_details=False):
    """
...
def download_file_from_google_drive(id: str, destination: str, file_size: int):
    ...
    def save_response_content(response, destination, file_size):
        ...
        with open(destination, "wb") as file:
            ...
            with tqdm(total=pieces, desc='{} Downloading Dataset...'.format(
                    datetime.today().strftime('%Y-%m-%d %H:%M:%S')), unit='B') as pbar:
